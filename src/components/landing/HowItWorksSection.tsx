import { UserPlus, Building, PackageCheck } from "lucide-react";
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            {steps.map((step, index) => (
              ...
              >
                {/* Step number circle */}
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center mx-auto mb-6 shadow-lg relative z-10">
                  <step.icon className="w-10 h-10 text-primary-foreground" />
                </div>

                {/* Number badge */}
                <div className="absolute top-0 right-1/2 translate-x-12 -translate-y-1 w-8 h-8 rounded-full bg-accent text-accent-foreground font-bold flex items-center justify-center text-sm shadow-md">
                  {step.number}
                </div>
